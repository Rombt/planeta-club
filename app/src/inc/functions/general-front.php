...
{
	global $rmbt_pc_options;

	if (!class_exists('Redux') || !isset($rmbt_pc_options[$id_field]) || $rmbt_pc_options[$id_field] === '') {
		return esc_url($custom_default_url !== '' ? $custom_default_url : false);
	}

	if (isset($rmbt_pc_options[$id_field])) {
		if (stripos($rmbt_pc_options[$id_field], get_site_url()) === 0) {
			return $rmbt_pc_options[$id_field];
		} else {
			$clear_url = str_replace($_SERVER['SERVER_NAME'] . '/', '', $rmbt_pc_options[$id_field]);
...
function rmbt_redux_get_pic_url($id_field_pic, $custom_default_url = '')
{
	...
	if (!class_exists('Redux') || !isset($rmbt_pc_options[$id_field_pic]['url']) || $rmbt_pc_options[$id_field_pic]['url'] === '') {
		return esc_url($custom_default_url !== '' ? $custom_default_url : false);
	}

	if (isset($rmbt_pc_options[$id_field_pic]['url'])) {
		if (stripos($rmbt_pc_options[$id_field_pic]['url'], get_site_url()) === 0) {
			return $rmbt_pc_options[$id_field_pic]['url'];
		} else {
			$clear_url = str_replace($_SERVER['SERVER_NAME'] . '/', '', $rmbt_pc_options[$id_field_pic]['url']);
			return esc_url(get_template_directory_uri() . $clear_url);
		}
	}
}
...
function rmbt_get_redux_field($id_field, $kses = false)
{
	...
	if ($kses) {
		return class_exists('ReduxFramework') ? wp_kses($rmbt_pc_options[$id_field], 'post') : "";
	}
	return class_exists('ReduxFramework') ? esc_html__($rmbt_pc_options[$id_field]) : "";
}
...
function get_arr_names_cat_equip()
{
	...
	$arr_redux_fields = array_filter($rmbt_pc_options, function ($var) {
